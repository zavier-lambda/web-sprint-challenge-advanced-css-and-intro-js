...
function get20s(arr) {
  ...
  for (let i = 0; i < arr.length; i++) {
    finder = arr[i].years.split(" ");
    if (Number(finder[0]) >= 1900 && Number(finder[0]) <= 2000 || Number(finder[2]) >= 1900 && Number(finder[2]) <= 2000) {
      artistList.push(arr[i].name);
    }
...
addArtist(21, "Zavier Guadalupe", "1997 - present", "Web Design", "American", "He was a really great guy.")
